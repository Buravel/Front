...
function BookmarkDetail({ match }) {
    ...
    const deletepost = (k) => {
        for (let i = 0; i < checkedInputs.length; i++) {
            axios.delete(
                `http://34.64.93.115/bookmark/post/${checkedInputs[i]}`,
            );
        }
        alert('포스트가 삭제되었습니다');
        setBookmarks(
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                ...
                setBookmarkstitle(
                    response.data._embedded.bookmarkResponseDtoList,
                );
            } catch (e) {
                setError(e);
            }
        ...
        <>
            ...
            <div className="bkDtBackground">
                ...
                <div className="btDTpage">
                    ...
                    <button onClick={toggleBmarkPopup}>
                        <Bicon picture="bringtoPlan" className="bkDtPlanEdit" />
                    </button>
                    <button onClick={deletepost}>
                        <Bicon
                            picture="BookmarkDeleteButton"
                            className="bkDtDelete"
                        />
                    </button>
